...
async function updateStats() {
  try {
    ...
    // CALCOLO PESO FONT VARIABILE
    const maxWeight = 900;
    const minWeight = 100;

    const hourWeight = minWeight + ((hrs / 24) * (maxWeight - minWeight));
    const minuteWeight = minWeight + ((mins / 60) * (maxWeight - minWeight));
    const secondWeight = minWeight + ((secs / 60) * (maxWeight - minWeight));

    // APPLICA PESO AL TESTO
    document.getElementById("hours").style.fontVariationSettings = `"wght" ${hourWeight}`;
    document.getElementById("minutes").style.fontVariationSettings = `"wght" ${minuteWeight}`;
    document.getElementById("seconds").style.fontVariationSettings = `"wght" ${secondWeight}`;
  } catch (err) {
